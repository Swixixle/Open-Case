...
**BASE_URL hardening:** If **`ENV=production`**, startup **exits** when **`BASE_URL`** is empty or contains `localhost`. Otherwise only **warns**. **`CONGRESS_API_KEY`** missing produces a startup warning (optional; helps name search and Senate vote matching).
...
## BLOCKED BY
Live verification: **network** for Senate.gov vote XML; optional **CONGRESS_API_KEY** for Congress.gov + identity matching. App + idempotency script must share the same **`open_case.db`** path.
